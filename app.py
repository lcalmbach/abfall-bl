...
def get_general_text(df_kg, df_t, gemeinde):
    ...
    text = f"""Die Gemeinde {gemeinde} hat im Jahr {YEARS[-1]} insgesamt {last_year_waste_t: .1f} Tonnen Abfall 
...
def get_category_text(df_t, df_kg, kategorie, gemeinde):
    ...
    if last_year_t > 0:
        ...
        {last_year_kg: .1f} kg/Kopf. Im {YEARS[0]} waren es {first_year_kg} kg/Kopf Im Ranking der Gemeinden erreicht {gemeinde} beim pro-Kopf-Verbrauch Platz {rank: .0f}.
        """
    else:
        text = ""
    return text
